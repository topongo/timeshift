...
public class SnapshotRepo : GLib.Object{
	...
	public bool unlock_and_mount_devices(){
		...
		if (btrfs_mode){
			...
			if (repo_subvolumes.has_key("@var")){

				var subvol = repo_subvolumes["@var"];

				if (subvol.device_uuid != device.uuid){

					// @var is on a separate device
					device_var = subvol.get_device();

					mount_paths["@var"] = unlock_and_mount_device(device_var, "/run/timeshift/backup-var");

					if (mount_paths["@var"].length == 0){
						return false;
					}
				}
			}
