...
public class Main : GLib.Object{
	...
	public bool include_btrfs_h_v_for_backup = false;
	public bool include_btrfs_h_v_for_restore = false;
	...
	public bool check_btrfs_layout_system(Gtk.Window? win = null){
		...
		if (include_btrfs_h_v_for_backup){
			supported =  supported && sys_subvolumes.has_key("@home") && sys_subvolumes.has_key("@var");
		}

		if (!supported){
			string msg = _("The system partition has an unsupported subvolume layout.") + " ";
			msg += _("Only ubuntu-type layouts with @, @home and @var subvolumes are currently supported.") + "\n\n";
	...
	public bool check_btrfs_layout(Device? dev_root, Device? dev_home, Device? dev_var, bool unlock){
		...
		if ((dev_root != null) && (dev_root.fstype == "btrfs")){
			...
			if ((dev_home != null) && (dev_home.fstype == "btrfs")){
				...
				if (dev_home != dev_root){
					...
					if (include_btrfs_h_v_for_backup){
						supported = supported && check_btrfs_volume(dev_home, "@home", unlock) && check_btrfs_volume(dev_var, "@var", unlock);
					}
				}
				else{
					if (include_btrfs_h_v_for_backup){
						supported = supported && check_btrfs_volume(dev_root, "@,@home,@var", unlock);
					}
					else{
						supported = supported && check_btrfs_volume(dev_root, "@", unlock);
					}
	...
	private Snapshot? create_snapshot_with_btrfs(string tag, DateTime dt_created){
		...
		if ((repo.device_var != null) && (repo.device_var.uuid != repo.device.uuid)){
			log_msg(_("Saving to device") + ": %s".printf(repo.device_var.device) + ", " + _("mounted at path") + ": %s".printf(repo.mount_paths["@var"]));
		}
		...
		if (include_btrfs_h_v_for_backup){
			...
			subvol_names = new string[] { "@", "@home", "@var" };
		}
		...
		foreach(var subvol_name in subvol_names){
			...
			else if (dst_path.has_suffix("/@var/@var")){
				dst_path = dst_path.replace("/@var/@var", "/@var");
			}
	...
	public Device? dst_var{
		get {
			foreach(var mnt in mount_list){
				if (mnt.mount_point == "/var"){
					return mnt.device;
				}
			}
			return null;
	...
	public void init_mount_list(){
		...
		if (!var_found){
			log_debug("added null entry: /var");
			mount_list.add(new MountEntry(null, "/var", "")); // add var entry
		}
	...
	public bool restore_snapshot(Gtk.Window? parent_win){
		...
		if (btrfs_mode){
			...
			if (include_btrfs_h_v_for_restore && (repo.mount_paths["@home"].length == 0)){
				log_error(_("BTRFS device is not mounted") + ": @home");
				return false;
			}
			if (include_btrfs_h_v_for_restore && (repo.mount_paths["@var"].length == 0)){
				log_error(_("BTRFS device is not mounted") + ": @var");
				return false;
			}
		}
		else{
	...
	public void get_restore_messages(bool formatted,
		...
		foreach(var entry in mount_list){
			...
			if (btrfs_mode){
				...
				if ((entry.subvolume_name() == "@home") && !include_btrfs_h_v_for_restore){ continue; }
				if ((entry.subvolume_name() == "@var") && !include_btrfs_h_v_for_restore){ continue; }
			}
	...
	public bool restore_execute_btrfs(){
		...
		foreach(var subvol in snapshot_to_restore.subvolumes.values){
			...
			if ((subvol.name == "@home") && !include_btrfs_h_v_for_restore){ continue; }
			if ((subvol.name == "@var") && !include_btrfs_h_v_for_restore){ continue; }
	...
	public bool create_pre_restore_snapshot_btrfs(){
		...
		if (restore_current_system){
			...
			if (found){
				...
				if (include_btrfs_h_v_for_restore && sys_subvolumes.has_key("@home") && snapshot_to_restore.subvolumes.has_key("@home")){
					sys_subvolumes["@home"].remove();
					log_msg(_("Deleted subvolume") + ": @home");
				}
				if (include_btrfs_h_v_for_restore && sys_subvolumes.has_key("@var") && snapshot_to_restore.subvolumes.has_key("@var")){
					sys_subvolumes["@var"].remove();
					log_msg(_("Deleted subvolume") + ": @var");
				}
		...
		if (create_pre_restore_backup){
			...
			var subvol_names = new string[] { "@" };
			if (include_btrfs_h_v_for_restore){
				subvol_names = new string[] { "@","@home", "@var" };
			}
			...
			foreach(string subvol_name in subvol_names){
				...
				if (status != 0){
					...
					Device subvol_dev;
					if(subvol_name == "@"){
						subvol_dev = repo.device;
					}
					else if(subvol_name == "@home"){
						subvol_dev = repo.device_home;
					}
					else{
						subvol_dev = repo.device_var;
					}
	...
	public void save_app_config(){
		...
		config.set_string_member("include_btrfs_h_v_for_backup", include_btrfs_h_v_for_backup.to_string());
		config.set_string_member("include_btrfs_h_v_for_restore", include_btrfs_h_v_for_restore.to_string());
	...
	public void load_app_config(){
		...
		if (config.has_member("include_btrfs_home")){
			include_btrfs_h_v_for_backup = json_get_bool(config, "include_btrfs_home", include_btrfs_h_v_for_backup);
		}
		else{
			include_btrfs_h_v_for_backup = json_get_bool(config, "include_btrfs_h_v_for_backup", include_btrfs_h_v_for_backup);
		}

		include_btrfs_h_v_for_restore = json_get_bool(config, "include_btrfs_h_v_for_restore", include_btrfs_h_v_for_restore);
	...
	public void detect_system_devices(){
		...
		foreach(var pi in partitions){
			...
			foreach(var mp in pi.mount_points){
				...
				if (mp.mount_point == "/var"){
					sys_var = pi;
					if ((app_mode == "")||(LOG_DEBUG)){
						string txt = _("/var is mapped to device") + ": %s, UUID=%s".printf(pi.device,pi.uuid);
						if (mp.subvolume_name().length > 0){
							txt += ", subvol=%s".printf(mp.subvolume_name());
						}
						log_debug(txt);
	...
	public bool mount_target_devices(Gtk.Window? parent_win = null){
		...
		if (dst_root.fstype == "btrfs"){
			...
			bool supported = check_btrfs_layout(dst_root, dst_home, dst_var, false);
	...
	public bool query_subvolume_ids(){
		...
		if (((repo.device_home != null) && (repo.device.uuid != repo.device_home.uuid)) || ((repo.device_var != null) && (repo.device.uuid != repo.device_var.uuid))){
			ok = ok && query_subvolume_id("@home") && query_subvolume_id("@var");
		}
		return ok;
	}
		...
		* */
		...
		foreach(string line in std_out.split("\n")){
			...
			if ((sys_subvolumes.size > 0) && line.has_suffix(sys_subvolumes["@"].path.replace(repo.mount_paths["@"] + "/"," "))){
				...
				&& sys_subvolumes.has_key("@var")
				&& line.has_suffix(sys_subvolumes["@var"].path.replace(repo.mount_paths["@var"] + "/"," "))){

				subvol = sys_subvolumes["@var"];
			}
			else {
				foreach(var bak in repo.snapshots){
	...
	public bool query_subvolume_quotas(){
		...
		if (repo.device.uuid != repo.device_home.uuid || repo.device.uuid != repo.device_var.uuid){
			ok = ok && query_subvolume_quota("@home") && query_subvolume_quota("@var");
		}
		return ok;
	}
	...
	public bool query_subvolume_quota(string subvol_name){
		...
		foreach(string line in std_out.split("\n")){
			...
			if ((sys_subvolumes.size > 0) && (sys_subvolumes["@"].id == subvol_id)){
				...
				&& sys_subvolumes.has_key("@var")
				&& (sys_subvolumes["@var"].id == subvol_id)){
	...
	public bool enable_subvolume_quotas(){
		...
		if (repo.device.uuid != repo.device_home.uuid || repo.device.uuid != repo.device_var.uuid){
			ok = ok && enable_subvolume_quota("@home") && enable_subvolume_quota("@var");
		}
		if (ok){
	...
	public bool rescan_subvolume_quotas(){
		...
		if (repo.device.uuid != repo.device_home.uuid && repo.device.uuid != repo.device_var.uuid){
			ok = ok && rescan_subvolume_quota("@home") && rescan_subvolume_quota("@var");
		}
		if (ok){
			log_msg(_("Enabled subvolume quota support"));
		}
