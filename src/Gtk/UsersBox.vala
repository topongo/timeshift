...
class UsersBox : Gtk.Box{
	...
	private Gtk.CheckButton chk_include_btrfs_h_v;
	...
	public UsersBox (Gtk.Window _parent_window, ExcludeBox _exclude_box, bool _restore_mode) {
		...
		init_btrfs_h_v_option(box_btrfs);
	...
	private void init_btrfs_h_v_option(Gtk.Box box){

		if (restore_mode){

			chk_include_btrfs_h_v = new Gtk.CheckButton.with_label(_("Restore @home and @var subvolumes"));

			box.add(chk_include_btrfs_h_v);

			chk_include_btrfs_h_v.toggled.connect(()=>{
				App.include_btrfs_h_v_for_restore = chk_include_btrfs_h_v.active;
			});

		}
		else {

			chk_include_btrfs_h_v = new Gtk.CheckButton.with_label(_("Include @home and @var subvolumes in backups"));

			box.add(chk_include_btrfs_h_v);

			chk_include_btrfs_h_v.toggled.connect(()=>{
				App.include_btrfs_h_v_for_backup = chk_include_btrfs_h_v.active;
			});
		}
	}
	...
	public void refresh(){
		...
		if (App.btrfs_mode){
			...
			if (restore_mode){
				chk_include_btrfs_h_v.active = App.include_btrfs_h_v_for_restore;
			}
			else{
				chk_include_btrfs_h_v.active = App.include_btrfs_h_v_for_backup;
			}
		}
		else{
			lbl_message.show();
